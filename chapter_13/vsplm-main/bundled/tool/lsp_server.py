...
LSP_SERVER = server.LanguageServer(
    name="VS PLM", version="<server version>", max_workers=MAX_WORKERS
)
...
def _run_tool_on_document(
    ...
    if use_path:
        ...
        with utils.substitute_attr(sys, "path", sys.path[:]):
            try:
                # TODO: `utils.run_module` is equivalent to running `python -m vsplm`.
...
if __name__ == "__main__":
    LSP_SERVER.start_io()
    LSP_SERVER.log_info(f"Starting {TOOL_DISPLAY} Language Server")
